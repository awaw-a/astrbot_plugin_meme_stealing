...
- 本地 FastAPI 管理面板，默认 `127.0.0.1` 仅本机访问；配置为 `0.0.0.0` 时允许局域网访问；所有访问都需要 `admin_token` 鉴权。
...
面板功能包括图片预览、搜索、编辑 description/tags/emotion、启用/禁用、待审核状态和删除。不要把 `panel_host` 配成 `0.0.0.0` 后直接暴露到公网。

`panel_host` 可选：

- `127.0.0.1`：仅 AstrBot 所在机器本机访问。
- `0.0.0.0`：允许同一局域网设备通过 AstrBot 机器的局域网 IP 访问。
