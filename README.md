...
所有聊天指令都只有 `admin_users` 中配置的管理员 QQ 号可以使用。`admin_users` 留空时任何人都不能使用插件指令，请至少填写一个管理员 QQ 号。
...
面板功能包括图片预览、来源群聊、保存时间、搜索、编辑 description/tags/emotion、启用/禁用、待审核状态、单张删除和批量删除。
